...
<div class="modal-header">
    <h5 class="modal-title" id="modal-edit-detail-title">
        {{ 'Radiologi MCU ID ' . $participant->code . ' | ' . $participant->name }}
    </h5>
    ...
    <div class="row">
        ...
        <div class="col-md-6 mb-2">
            <div class="form-group">
                ...
                <textarea name="diafragma_sinus" id="diafragma_sinus" cols="30" rows="2"
                    class="form-control">{{ $radiologi?->diafragma_sinus }}</textarea>
            </div>
        </div>
        <div class="col-md-6 mb-2">
            <div class="form-group">
                <label for="" class="form-label">Pulmo</label>
                <textarea name="pulmo" id="pulmo" cols="30" rows="2"
                    class="form-control">{{ $radiologi?->pulmo }}</textarea>
            </div>
        </div>
        <div class="col-md-6 mb-2">
            <div class="form-group">
                <label for="" class="form-label">Kesan</label>
                <textarea name="kesan" id="kesan" cols="30" rows="2"
                    class="form-control">{{ $radiologi?->kesan }}</textarea>
            </div>
        </div>
...
<div class="modal-footer">
    ...
    <button type="button" class="btn btn-info"
        onclick="window.open('{{ route('report.radiologi', $participant->id) }}', '', 'toolbar=yes,scrollbars=yes,resizable=yes,width=900,height=600');">Print</button>
...
<script>
    ...
    @if($radiologi?->employee?->id)
        selectElement.value = "{{ $radiologi?->employee?->id }}";
    @else

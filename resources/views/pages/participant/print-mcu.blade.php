...
@section('content')
    <style>
        .table-responsive table.textkecil {
            ...
            /* Atur ukuran font kecil dan prioritaskan */
        }

        .table-responsive th.textkecil,
        .table-responsive td.textkecil {
            font-size: 10px !important;
            /* Pastikan ukuran font pada header dan sel tabel */
        }
    ...
    <div class="card">
        ...
        <div class="card-body">
                    ...
                    <div class="col-md-3">
                        ...
                        <select name="filter[contract_id]" id="contract_id" class="form-select">
                            ...
                            @foreach ($client->contracts as $item)
                                <option value="{{ $item->id }}" {{ session('contract_id') == $item->id ? 'selected' : '' }}>
                                    {{ $item->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            <div class="row mt-2">
                <div class="col-md-3">
                    <input type="text" id="start" class="form-sm form-control" value="1" placeholder="Dari No" />
                </div>
                <div class="col-md-3">
                    <input type="text" id="end" class="form-sm form-control " value="50" placeholder="Sampai No" />
                </div>
                <div class="col-md-6">
                    <div class="btn-group" role="group" aria-label="First group">
                        <button type="button" class="btn btn-sm btn-outline-info waves-effect import" data-type="identitas"
                            data-url="{{ route('multiple.identitas') }}">
                            Stiker
                        </button>
                        ...
                        <button type="button" class="btn btn-sm btn-outline-primary waves-effect import"
                            data-url="{{ route('multiple.pemFisik') }}">
                            Pem. Fisik
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-primary waves-effect import"
                            data-url="{{ route('multiple.laboratorium') }}">
                            Lab
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-warning waves-effect import"
                            data-url="{{ route('multiple.radiologi') }}">
                            Rad
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-warning waves-effect import">
                            Audiometri
                        ...
                        <button type="button" class="btn btn-sm btn-outline-success waves-effect import"
                            data-url="{{ route('multiple.ekg') }}">
                            EKG
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-success waves-effect import"
                            data-url="{{ route('multiple.spirometri') }}">
                            Spiro
                        </button>
                        <button type="button" class="btn btn-sm btn-outline-success waves-effect import"
                            data-url="{{ route('multiple.rectal') }}">
                            Rectal
                        </button>
                    </div>
                </div>
                    ...
                    <tbody>
                        @foreach ($data as $key => $item)
                            <tr>
                                ...
                                <td class="border-bottom border-top text-left">{{$item->name}}
                                    [{{$item->gender}}]<br>{{$item->birthday}}</td>
                                <td class="border-bottom border-top text-left">{{$item->plan_name ?? "-"}}</td>
                                <td class="border-bottom border-top text-center">
                                    <a href="#"
                                        class="text-center btn btn-sm btn-info btn-biru font-weight-medium btn-circle printStiker"
                                        data-bs-toggle="tooltip" data-bs-placement="bottom"
                                        data-bs-original-title="Print Stiker"
                                ...
                                <td class="border-bottom border-top text-center hilang">
                                    ...
                                    @if($item?->validateDoctor)
                                        <button type="button" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                            data-bs-original-title="Print Tanda Vital"
                                            class="text-center btn btn-sm btn-info font-weight-medium btn-circle printFisik"
                                            onclick="window.open('{{ route('report.resume', $item->id) }}', '', 'toolbar=yes,scrollbars=yes,resizable=yes,width=900,height=600');">
                                ...
                                <td class="border-bottom border-top text-center hilang">
                                    @if($item->pemeriksaanFisik?->selesai)
                                        <button type="button" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                            data-bs-original-title="Print Pemeriksaan Fisik"
                                            class="text-center btn btn-sm btn-primary font-weight-medium btn-circle printFisik"
                                            onclick="window.open('{{ route('report.pemeriksaan.fisik', $item->id) }}', '', 'toolbar=yes,scrollbars=yes,resizable=yes,width=900,height=600');">
                                            <i class="fa fa-print" aria-hidden="true"></i></button>
                                    @else
                                        <span class="text-center text-danger">TIDAK</span>
                                    @endif
                                ...
                                <td class="border-bottom border-top text-left hilang">
                                    @if($item->laboratorium?->selesai)
                                        <button type="button" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                            data-bs-original-title="Print Laboratorium"
                                            class="text-center btn btn-sm btn-primary font-weight-medium btn-circle laboratorium"
                                            onclick="window.open('{{ route('report.laboratorium', $item->id) }}', '', 'toolbar=yes,scrollbars=yes,resizable=yes,width=900,height=600');">
                                            <i class="fa fa-print" aria-hidden="true"></i></button>
                                    @else
                                        <span class="text-center text-danger">TIDAK</span>
                                    @endif
                                ...
                                <td class="border-bottom border-top text-left hilang">
                                    @if($item->radiologi?->selesai)
                                        <button type="button" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                            data-bs-original-title="Print Radiologi"
                                            class="text-center btn btn-sm btn-warning font-weight-medium btn-circle printRad"
                                            onclick="window.open('{{ route('report.radiologi', $item->id) }}', '', 'toolbar=yes,scrollbars=yes,resizable=yes,width=900,height=600');">
                                            <i class="fa fa-print" aria-hidden="true"></i></button>
                                    @else
                                        <span class="text-center text-danger">TIDAK</span>
                                    @endif
                                ...
                                <td class="border-bottom border-top text-left hilang">
                                    @if($item->audiometri?->selesai)
                                        <button type="button" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                            data-bs-original-title="Print audiometri"
                                            class="text-center btn btn-sm btn-warning font-weight-medium btn-circle printRad"
                                            onclick="window.open('{{ route('report.audiometri', $item->id) }}', '', 'toolbar=yes,scrollbars=yes,resizable=yes,width=900,height=600');">
                                            <i class="fa fa-print" aria-hidden="true"></i></button>
                                    @else
                                        <span class="text-center text-danger">TIDAK</span>
                                    @endif
                                ...
                                <td class="border-bottom border-top text-left hilang">
                                    @if($item->ekg?->selesai)
                                        <button type="button" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                            data-bs-original-title="Print EKG"
                                            class="text-center btn btn-sm btn-success font-weight-medium btn-circle printEkg"
                                            onclick="window.open('{{ route('report.ekg', $item->id) }}', '', 'toolbar=yes,scrollbars=yes,resizable=yes,width=900,height=600');">
                                            <i class="fa fa-print" aria-hidden="true"></i></button>
                                    @else
                                        <span class="text-center text-danger">TIDAK</span>
                                    @endif
                                ...
                                <td class="border-bottom border-top text-left hilang">
                                    @if($item->spirometri?->selesai)
                                        <button type="button" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                            data-bs-original-title="Print Spirometri"
                                            class="text-center btn btn-sm btn-success font-weight-medium btn-circle printEkg"
                                            onclick="window.open('{{ route('report.spirometri', $item->id) }}', '', 'toolbar=yes,scrollbars=yes,resizable=yes,width=900,height=600');">
                                            <i class="fa fa-print" aria-hidden="true"></i></button>
                                    @else
                                        <span class="text-center text-danger">TIDAK</span>
                                    @endif
                                ...
                                <td class="border-bottom border-top text-left hilang">
                                    @if($item->rectal?->selesai)
                                        <button type="button" data-bs-toggle="tooltip" data-bs-placement="bottom"
                                            data-bs-original-title="Print Rectal"
                                            class="text-center btn btn-sm btn-success font-weight-medium btn-circle printEkg"
                                            onclick="window.open('{{ route('report.rectal', $item->id) }}', '', 'toolbar=yes,scrollbars=yes,resizable=yes,width=900,height=600');">
                                            <i class="fa fa-print" aria-hidden="true"></i></button>
                                    @else
                                        <span class="text-center text-danger">TIDAK</span>
                                    @endif
